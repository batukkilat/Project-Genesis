...
- **Phase 6.5 — The experiment loop** (underway, runs alongside Phase 6): run scoring (`genesis score` → one RON record: identity stamp + final/peak Observer aggregates + the exit-criterion scalar), sweep driver (`genesis sweep` → per-run records + a comparison table; [sweeps/shipped-packs.ron](sweeps/shipped-packs.ron) is the baseline corpus, findings in [docs/research/sweeps/2026-07-13-shipped-packs.md](docs/research/sweeps/2026-07-13-shipped-packs.md)), and the evolutionary search ([docs/research/search-design.md](docs/research/search-design.md)): schema-bounded mutation operators with RON ancestry sidecars (`genesis mutate`) and the generation loop (`genesis search` — screen/confirm evaluation, saturating-product fitness, deterministic bond-cap circuit breaker; a finished search reproduces byte-for-byte from its spec).
...
| `genesis-headless` | CLI: run (with `--report` diagnostics, `--observer` config, `--timeline` RON dump), verify determinism, bench, branch (fork a save + ancestry record), score (one RON record per run), sweep (batch scoring + comparison table), mutate (schema-bounded world mutation + ancestry), search (the evolutionary loop), init-config, init-rules. |
...
cargo run -p genesis-headless --release -- sweep --spec sweeps/shipped-packs.ron --out sweep-results
cargo run -p genesis-headless --release -- search --spec sweeps/search-01.ron --out search-results
